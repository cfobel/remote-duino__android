...
public class RemoteDuinoActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            Toast.makeText(this, "[onCreate] Unexpected exception: " + e,
    ...
    void update_codes_list() {
         ...
         */
        /*
         * TODO: Change layout of buttons: -Grid? -Configurable (serializable)
         * layout? XML?
         */
        LinearLayout ll = (LinearLayout) findViewById(R.id.llo__remote_buttons);
        ll.removeAllViews();
        for(RemoteCommand cmd : cmd_manager.get_commands()) {
            final RemoteButton temp = new RemoteButton(this, cmd_manager,
                    mIRDevice, cmd);
            ll.addView(temp);
        }
    }
    ...
    public void set_label(String label) {
        /*
         * This function sets the temporary label value to be used when learning
         * a remote command. This may seem a little awkward, but is required
         * since the call-back needs somewhere to store state until the command
    ...
    private void showLearnCode() {
         ...
         * wrong, that appeared to make this necessary. Shouldn't hurt anything
         * for now though...
         */
        final RemoteDuinoActivity thisActivity = this;
        AlertHelper.showTextInputAlert(this, "Button Name:",
                new AlertOnClickI() {
                    public void onClick(DialogInterface dialog, TextView input) {
                        /*
                         * TODO: (see above) We may be able to simply use a
                         * local variable for 'label'. I seem to remember having
                         * issues with that, but once again, it may have been an
                         * unrelated issue that seemed to make this necessary.
                         */
                        thisActivity.set_label(input.getText().toString()
                                .trim());
                        RemoteCommand cmd;
                        try {
                            ...
                            Toast.makeText(
                                    thisActivity,
                                    "Added code to local list: " + cmd.code
                                            + ", " + cmd.protocol,
                                    Toast.LENGTH_SHORT).show();
                            thisActivity.update_codes_list();
                        } catch(Exception e) {
                            Toast.makeText(thisActivity, e.getMessage(),
                                    Toast.LENGTH_SHORT).show();
                            e.printStackTrace();
                        }
                    }
