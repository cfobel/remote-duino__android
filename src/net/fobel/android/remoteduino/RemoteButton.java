...
public class RemoteButton extends Button {
	...
	public RemoteButton(final RemoteDuinoActivity activity, RemoteCommandManager cmdManager, WebArduinoIRDevice irDevice, RemoteCommand cmd) {
		super(activity);
				...
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				        ...
				        case 2:
				        	...
				        	break;
				        default:
				        	Toast.makeText(mActivity, "Unknown command", Toast.LENGTH_SHORT).show();	
				        }
				        
				    }

					private void renameButton() {
						AlertHelper.showTextInputAlert(mActivity, "Rename: " + mCommand.label, new AlertOnClickI() {
				    		public void onClick(DialogInterface dialog, TextView input) {
				    			String newButtonText = input.getText().toString().trim();
			    				...
			    				Toast.makeText(mActivity, oldButtonText + " renamed to " + newButtonText, Toast.LENGTH_SHORT).show();	
				    		}
				    	});
					}

					private void relearnButton() {
						Toast.makeText(mActivity, "Relearn doesn't work yet.", Toast.LENGTH_SHORT).show();
					}

					private void deleteButton(final RemoteButton thisButton, final String labelText) {
						mCommandManager.remove(thisButton.mCommand);
						mActivity.update_codes_list();
						Toast.makeText(mActivity, labelText + " deleted", Toast.LENGTH_SHORT).show();
					}
				});
